...
public class OrientadorDAO {
	...
	public int inserir(OrientadorDTO orientadorDTO) throws PersistenciaException {
		int chave = 0;
		try{
			...
			String sql ="INSERT INTO orientador(NomeOrientador, CPFOrientador, SenhaOrientador)"
			+ " VALUES(?, ?, ?);";
			
			PreparedStatement statement = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
						
			statement.setString(1, orientadorDTO.getNomeOrientador());
			statement.setString(2, orientadorDTO.getCPFOrientador());
			statement.setString(3, orientadorDTO.getSenhaOrientador());
	...
	public void atualizar(OrientadorDTO OrientadorDTO) throws PersistenciaException {
		try{
			Connection connection =  ConexaoUtil.getInstance().getConnection();
			...
			Statement.setString(2, OrientadorDTO.getCPFOrientador());
			Statement.setString(3, OrientadorDTO.getSenhaOrientador());
			Statement.setInt(4, OrientadorDTO.getIDOrientador());
	...
	public void deletartudo() throws PersistenciaException{
		try{
			Connection connection = ConexaoUtil.getInstance().getConnection();
			String sql = "DELETE FROM orientador;";
	...
	public List<OrientadorDTO> listarTodos() throws PersistenciaException {
		...
		try{
			...
			String sql = "SELECT * FROM orientador;";
			 ...
			 while(resultSet.next()){ 
				 ...
				 orientadorDTO.setIDOrientador(resultSet.getInt(1));
				 orientadorDTO.setNomeOrientador(resultSet.getString(2));
				 orientadorDTO.setCPFOrientador(resultSet.getString(3));
				 orientadorDTO.setSenhaOrientador(resultSet.getString(4));
		
				 listaOrientador.add(orientadorDTO);
			 }
			 connection.close();
	...
	public OrientadorDTO buscarOrientadorPoID(int id) throws PersistenciaException {
		 OrientadorDTO orientadorDTO = null;
		try{
			Connection connection = ConexaoUtil.getInstance().getConnection();
			
			String sql = "SELECT * FROM orientador WHERE IDOrientador = ?;";
	...
	public OrientadorDTO buscarOrientadorPorCPF(String cpf) throws PersistenciaException {
		 OrientadorDTO orientadorDTO = null;
		try{
			Connection connection = ConexaoUtil.getInstance().getConnection();
			
			String sql = "SELECT * FROM orientador WHERE CPFOrientador = ?;";
	...
	public void gerarCertificadoOrientador(int idProjeto, int idEvento, String diretores) {
		File file = null;
		...
		try {
			...
			List<EventoOrientadorDTO> eventoOrientadores = new ArrayList<EventoOrientadorDTO>();
			eventoOrientadores = eventoDB.listarTodosEventoOrientador(idEvento);
			List<ProjetoOrientadorDTO> projetoOrientadores = new ArrayList<ProjetoOrientadorDTO>();
			projetoOrientadores = projetoDB.listarTodosProjetoOrientador(idProjeto);
			

			for (EventoOrientadorDTO eventoOrientador : eventoOrientadores) {
				EventoDTO evento = eventoDB.buscarEventoPoID(eventoOrientador.getIdEvento());
				int idProj = projetoDB.buscarProjetoOrientadorPoIDO(eventoOrientador.getIdOrientador());
				ProjetoDTO projeto = projetoDB.buscarProjetoPoID(idProj);
				OrientadorDTO orientador = buscarOrientadorPoID(eventoOrientador.getIdOrientador());
				...
				file = createPdf(projeto.getIDProjeto(), evento.getIDEvento(), orientador.getIDOrientador(),
						diretor1, diretor2, t.getToken());
				
				documentoDTO.setnomeDocumento(String.valueOf(orientador.getCPFOrientador()+evento.getNomeEvento()));
				byte[] doc = new byte[(int) (file).length()];
				java.io.DataInputStream is = new java.io.DataInputStream(new FileInputStream(file));
				is.readFully(doc);
				is.close();
				
				documentoDTO.setCpf(orientador.getCPFOrientador());
	...
	private File createPdf(int idProjeto, int idEvento, int idOrientador, int diretorUm, int diretorDois, String tokenCert) throws IOException, DocumentException, PersistenciaException {
		...
		OrientadorDTO orientador = buscarOrientadorPoID(idOrientador);
		...
		Document document = new Document(image, 72, 72, 90, 10); // esquerda,
																	...
																	// cima,
																	// baixo
		String nomeCaminho = orientador.getCPFOrientador()+evento.getNomeEvento()+".pdf";
		...
		String pDois = ", coordenador(a) do projeto “";
		String pTres = "”, apoiado por ";
		String pTresMeio = ", do Programa de Bolsas de ";
		String pQuatro =", orientou os (as) alunos(as) ";
		String pCinco = " na condição de bolsista(s) ";
		String pCincoMeio = "/";
		String pSeis = ", no período de ";
		String pSete = " a ";
		String pOito = ".";

		Paragraph p = new Paragraph();
		
		p = new Paragraph(pUm + nome + pDois + projeto + pTres +patrocinadores +pTresMeio +nomeprograma +pQuatro + alunos + pCinco 
				+ nomeprograma + pCincoMeio + patrocinadores + pSeis + dataInicio + pSete + dataFim + pOito,	 new Font(bf, 17));
