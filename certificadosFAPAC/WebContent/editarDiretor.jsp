...
<!-- <meta charset="UTF-8">-->
	...
	function Nova(){
	location.href="visualizarDiretor.jsp"
	}
	</script>
...
<header class="cd-main-header">
	...
	<main class="cd-main-content">
		...
		if(request.getParameter("n")!=null){
			...
			diretor = diretorDB.buscarDiretorPoID(Integer.parseInt(request.getParameter("n")));
		}
		if(request.getParameter("salvar")!=null){
			diretor = diretorDB.buscarDiretorPoID(Integer.parseInt(request.getParameter("id")));
			
			diretorLogic.setIDDiretor(diretor.getIDDiretor());
			diretorLogic.setNomeDiretor(request.getParameter("nomeDiretor"));
			diretorLogic.setCargoDiretor(request.getParameter("cargoDiretor"));
			diretorLogic.setNDecreto(request.getParameter("decretoDiretor"));
			
			diretorDB.atualizar(diretorLogic);
			response.sendRedirect("visualizarDiretor.jsp");
			%>
			<script type="text/javascript">
		...
		<form action="editarDiretor.jsp" accept-charset="iso-8859-1,utf-8" method="post">
		<h1>Editar diretor: <%=diretor.getNomeDiretor() %></h1>
		<element hidden><input type="text" name="id" value="<%=diretor.getIDDiretor()%>"></element>
		<label for="nome">Nome: &nbsp;&nbsp;&nbsp;</label>
		<input type="text" name="nomeDiretor" size="60" required="required" autofocus="autofocus" value="<%=diretor.getNomeDiretor()%>" >
		<br/><br/>
		<label for="cargo">Cargo: &nbsp;&nbsp;&nbsp;&nbsp;</label>
		<input type="text" name="cargoDiretor" size="60" required="required" autofocus="autofocus" value="<%=diretor.getCargoDiretor()%>" >
		<br/><br/>
		<label for="nome">Decreto: &nbsp;</label>
		<input type="text" name="decretoDiretor" size="9" required="required" autofocus="autofocus" value="<%=diretor.getNDecreto()%>" >
		<br/><br/><br/>
