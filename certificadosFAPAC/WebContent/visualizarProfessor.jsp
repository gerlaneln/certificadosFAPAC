<%@page import="br.acre.fapac.certificado.dto.CoorientadorDTO"%>
<%@page import="br.acre.fapac.certificado.dto.OrientadorDTO"%>
	...
	<main class="cd-main-content">
		...
		<div id="professor">
			<table class="table table-hover">
				...
				<%
				List<OrientadorDTO>orientadores = OrientadorDB.listarTodos();
				List<CoorientadorDTO>coorientadores = CoOrientadorDB.listarTodos();
				if(orientadores == null && coorientadores == null){
					%>
					<script type="text/javascript">
						alert("Não existem professores cadastrados")
					</script>
					<jsp:forward page="cadastroProfessorOrientador.jsp"></jsp:forward>
					<%
				}else{
					for(OrientadorDTO orientador : orientadores){
						int idEvento = eventoDB.buscarEventoOrientadorPoIDO(orientador.getIDOrientador());
						eventoLogic = eventoDB.buscarEventoPoID(idEvento);
						String nomeEvento = eventoLogic.getNomeEvento();
						
						int idProjeto = projetoDB.buscarProjetoOrientadorPoIDO(orientador.getIDOrientador());
						projetoLogic = projetoDB.buscarProjetoPoID(idProjeto);
						String nomeProjeto = projetoLogic.getTituloProjeto();
						%>
						<tr>
						<td>
						<label for="professor"><%=orientador.getNomeOrientador() %></label>
						</td>
						<td>
						<label for="coorientador">Não</label>
						</td>
						<td>
						<label for="eventoEdital"><%=nomeEvento.toString() %></label>
						</td>
						<td>
						<label for="projeto"><%=nomeProjeto.toString() %></label>
						</td>
						<td>
						<a href="editarProfessorOrientador.jsp?n=<%=orientador.getIDOrientador()%>"><img src="./resources/images/editar.png" width="25" height="25"></a>
						</td>
						<td>
						<a href="deletarProfessorOrientador.jsp?n=<%=orientador.getIDOrientador()%>"><img src="./resources/images/excluir.png" width="25" height="25"></a>
						</td>
						</tr>
						<%
					}
					for(CoorientadorDTO coorientador : coorientadores){
						int idEvento = eventoDB.buscarEventoCoorientadorPoIDO(coorientador.getIDCoorientador());
						eventoLogic = eventoDB.buscarEventoPoID(idEvento);
						String nomeEvento = eventoLogic.getNomeEvento();
						
						int idProjeto = projetoDB.buscarProjetoCoorientadorPoIDC(coorientador.getIDCoorientador());
						projetoLogic = projetoDB.buscarProjetoPoID(idProjeto);
						String nomeProjeto = projetoLogic.getTituloProjeto();
						%>
						<tr>
						<td>
						<label for="professor"><%=coorientador.getNomeCoorientador() %></label>
						</td>
						<td>
						<label for="coorientador">Sim</label>
						</td>
						<td>
						<label for="eventoEdital"><%=nomeEvento.toString() %></label>
						</td>
						...
						<td>
						<a href="editarProfessorCoorientador.jsp?n=<%=coorientador.getIDCoorientador()%>"><img src="./resources/images/editar.png" width="25" height="25"></a>
						</td>
						<td>
						<a href="deletarProfessorCoorientador.jsp?n=<%=coorientador.getIDCoorientador()%>"><img src="./resources/images/excluir.png" width="25" height="25"></a>
						</td>
						</tr>
						<%
					}
